	XULBinGoMultiSearchTabChrome.USLanguagePack= {
			...
			AddBINGOSearchDIV:function(doc,LinkNextUDiv){
				...
				if(doc.domain=="search.yahoo.com"){
					if(!elem){
						elem=doc.getElementById('web');
						if(elem){
							var parentDiv = elem.parentNode;
							parentDiv.insertBefore(LinkNextUDiv, elem);
						} 
			...
			AddUSSearchAssist:function(doc,LinkNextUDiv,keyword){
				...
				GoogleLink.setAttribute('href', 'https://www.google.com/search?q='+keyword);
				...
				YahooLink.setAttribute('href', 'http://search.yahoo.com/search?fr=www.linknextu.com&p='+keyword);
				TempTxt = doc.createTextNode('Yahoo!');
				...
				AmazonLink.setAttribute('href', 'http://www.amazon.com/s/ref=nb_sb_noss_1?tag=linknextucom-22&url=http://www.linknextu.com&field-keywords='+keyword);
